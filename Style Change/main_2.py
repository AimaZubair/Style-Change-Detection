...
def test(clf, train_x, train_y, train_positions, with_full_data_tfidf):
    ...
    predictions = get_breach_predictions(clf, test_x, test_y)
    t_end = time.time()
...
if config_local().get('persist_results', False): write_results_to_file(results)
...
#if model_dump:
    #estimator = MLP()
    #test_x  = 'problem-1.txt'
    #mytxt = test_x.read()
    #print(mytxt)
    #prediction = get_breach_predictions(estimator, text_x, change_predictions)
    #print(prediction)
  ...
  #  with open('model.pkl', 'rb') as file:
    ...
    #    test(loaded_model, train_x, train_y, train_positions, with_full_data_tfidf)
